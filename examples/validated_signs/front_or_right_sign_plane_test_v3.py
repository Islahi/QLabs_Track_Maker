...
# ================================================================
# Front-or-Right traffic sign - Plane test v3
# ================================================================
# Uses the approved multi-box symbol and the corrected 3-D box orientation.
# The pole is shifted behind the circular plate so it does not cover the
# visible sign face from the approaching-driver side.
# ================================================================
...
def rotation_matrix_to_euler_zyx(matrix):
    ...
    # Standard R = Rz(yaw) * Ry(pitch) * Rx(roll).
    if abs(r20) < 1.0 - 1e-8:
        pitch = -math.asin(r20)
        roll = math.atan2(r21, r22)
        yaw = math.atan2(r10, r00)
    else:
        # Gimbal-lock case, used by vertical symbol segments.
...
def segment_rotation(heading, p1, p2):
    ...
    # Local X axis follows the requested 2-D symbol stroke.
    x_axis = [
        n_left[0] * du / length,
        n_left[1] * du / length,
        dv / length,
    ...
    # Local Y is the thin box depth, normal to the sign face.
    y_axis = face_normal

    # Local Z becomes the stroke width direction in the sign face.
...
def spawn_face_segment(
    ...
    spawn_shape(
        ...
        rotation=rotation,
        scale=[
            length * 1.10,  # same overlap used in the approved ground test
            STROKE_DEPTH,
            width,
        ],
...
def spawn_front_or_right_symbol(shape, actor_ids, face_origin, heading):
    """Exact logical geometry from the approved ground test."""
    ...
    # Straight-ahead filled arrowhead.
    draw_arrow_head(
        shape,
        actor_ids,
        face_origin,
    ...
    # EXACT 20-segment quarter-circle from the approved ground test.
    branch_arc = arc_points(
        center_x=0.22,
        center_y=-0.03,
        radius=0.22,
        start_deg=180.0,
...
def spawn_front_or_right_sign(
    ...
    # Post.
    #
    # The visible artwork is placed on the front side of the plate at -t.
    # Shift the pole in the opposite direction (+t) so the pole sits behind
    # the plate rather than covering the face from the driver's viewpoint.
    t = tangent(heading)
    ...
    # Known-good plate orientation from the working speed-limit implementation.
    spawn_shape(
        shape,
        actor_ids,
    ...
    # White artwork just in front of the blue plate.
    face_origin = [
...
def main():
    ...
    print("This v3 test keeps the approved arc and moves the pole behind the plate.")
    ...
    print("Front-or-Right traffic sign v3 created.")
