...
from items.actors import ResizableSceneActorItem


class EnvironmentAssetItem(ResizableSceneActorItem):
    ...
    def length_px(self) -> float:
        return self.FOOTPRINT_LENGTH_M * self.actor_scale * PIXELS_PER_METER

    @property
    def width_px(self) -> float:
        return self.FOOTPRINT_WIDTH_M * self.actor_scale * PIXELS_PER_METER
    ...
    def resize_rect_local(self) -> QRectF:
        return self._asset_rect()

    def resize_from_handle(self, point: QPointF):
        base_half_length = self.FOOTPRINT_LENGTH_M * PIXELS_PER_METER / 2.0
        base_half_width = self.FOOTPRINT_WIDTH_M * PIXELS_PER_METER / 2.0
        scale_x = abs(float(point.x())) / max(base_half_length, 1e-6)
        scale_y = abs(float(point.y())) / max(base_half_width, 1e-6)
        self.prepareGeometryChange()
        self.actor_scale = max(0.05, min(100.0, max(scale_x, scale_y)))
    ...
    def selection_text(self) -> str:
        text = (
            ...
            + f"\nFootprint: {self.length_px / PIXELS_PER_METER:.2f} × "
            + f"{self.width_px / PIXELS_PER_METER:.2f} m"
            + "\nDrag the cyan corner handle to resize uniformly"
        )
        if self.auto_generated:
